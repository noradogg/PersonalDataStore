...
# log_data = get_from_jsonfile(dir_name + "/" + sp_name + ".json")
...
""" pdsに入れるテスト
    ...
    データベースに保存するためにはオプションに "-O" をつけて下のコードを有効に
"""
def insert(source_data: list):
    # mongodbの personal_data の pds_user_idのコレクション に保存
    if __debug__:
        print(source_data) # 確認用
    else:
        mongo_personaldata = Mongodb("personal_data", "personal_data_2")
        result = mongo_personaldata.insert(source_data)
